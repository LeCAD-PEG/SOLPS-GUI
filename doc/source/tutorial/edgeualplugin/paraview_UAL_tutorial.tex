...
Data arrays are holding data such as \textit{electron density/temperature} and \textit{ion density/temperature} scalars, which are used to color the blocks. When running the ReadUALedge plugin with the \textit{Multi-Block Inspector} the \textit{vtkBlockColors} layer is automatically selected, coloring the blocks with their specific block color. To select the wanted data layer navigate through \textbf{List of Data Arrays} as seen in Fig. \ref{fig:pv_data_arrays_list}. Examples are shown in Figures \ref{fig:pv_data_arrays_list_ne}, \ref{fig:pv_data_arrays_list_te} and \ref{fig:pv_data_arrays_list_te_core_sol}.
...
Note 1: If we have a long list od data layers and want to create sum of all of them the expression can be very long. Typing it manually can be frustrating, so it's advisable to use a short self written code to generate the expression. ParaView includes also its own Python Shell. The use of Python Shell in Paraview and example of code to generate the expression is explained in \textit{ParaView Python Shell} chapter.

Note 2: After the Python Calculator is run and until  applying the setting in Pipeline Browser there will be one block shown in Multi-Block Inspector, if activated, with the name $PythonCalculator1$. If we deselect this block, the ParaView will crash and it may also cause the ReadUALEdge plugin to fully unload, in which case it must be again manually loaded using Plugin Manager. 
...
Here we'll show an example solving the issue with very long Python Calculator Expressions. 
...
The Python code and part of its output is shown in Fig. \ref{fig:pv_python_shell3}.
...
The results of using the mentioned IDS database and generated expression are shown in Fig. \ref{fig:pv_python_calculator5}.
...
Plot Over Line filter is a flexible tool, used to create a plot such temperature change through the wall. 
...
It can be found at the same location as any other filter, navigating to $Filters \to Alphabetical \to Plot over Line$, as seen on Fig. \ref{fig:pv_pov1}. When selected, two white dots, representing the start and end location taken for the plot, connected with line will appear in the \textit{View Browser}, as seen on Fig. \ref{fig:pv_pov2}, which can be moved by clicking and dragging with the mouse pointer. When done, press the \textbf{Apply} button. 
...
After pressing the \textbf{Apply} button a new interface will open in the \textit{Pipeline Browser} and also the plot will appear on the \textit{View Browser}, as seen on Fig. \ref{fig:pv_pov3}. 

\begin{figure}[!htb] \centering
    \includegraphics[scale=0.5,trim=0cm 0cm 0cm 0cm,clip]{30_pov3}
    \caption{Plot example}
    \label{fig:pv_pov3}
\end{figure}

We can notice, that all the Data Arrays are being shown on the plot and also making some of them unclear because of the used  value range. 

There are many different settings to get the desired plot view, here a few of them will be shown.

To display only the selected Data Array plots, in the new \textit{Pipeline Browser} interface scroll down to \textbf{Series Parameters} and there the desired Data Arrays (and also showing coordinate changes from start to end point) can be chosen, as seen on Fig. \ref{fig:pv_pov4}. The range will automatically adjust to selected plot values.

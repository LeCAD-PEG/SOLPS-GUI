...
used to monitor and analyze simulation runs. Dashboard configuration should be
...
and creating accompanying Python action code. For example, with added
...
(eg. QTabWidget for input files editing) and do the widget initialization on
...
steps:
 ...
 2. Shared library is build by ``Makefile`` that was created from ``qmake``
    project file.
 3. SIP file needs to be written describing C++ code for Python bindings
 4. ``configure.py`` file needs to be updated with details of the files used
    ...
    project with clearly marked implementation sections that needs to be
    updated.
...
All these files can be seen in **solps-gui/src/examples/hello/** folder.

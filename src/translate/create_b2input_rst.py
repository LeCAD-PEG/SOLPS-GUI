...
def stringify_children(node):
    if node is not None:
        # node.getchildren() will be deprecated
        # using list(node)
        ...
        middlePart = list(chain(*([tostring(c).split()[0], c.tail] for c in list(node))))
        parts = ([node.text] +
                list(chain(*([tostring(c).split()[0], c.tail] for c in list(node)))) +
                [node.tail])

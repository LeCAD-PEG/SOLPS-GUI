...
class B2Vars:
    NumOfVars = 4
    b2ai, b2ah, b2ar, b2mn = range(NumOfVars)

    Name = {0: 'b2ai', 1: 'b2ah', 2: 'b2ar', 3: 'b2mn'}
    command = Name

    Values = {'b2ai': 0, 'b2ah': 1, 'b2ar': 2, 'b2mn': 3}
...
class B2InputFiles(TcshProcess):
    ...
    def prepareUserInterface(self):
        ...
        leftOver = B2Vars.NumOfVars - (B2Vars.NumOfVars // _n) * _n
        if leftOver > 0:
...
class B2Vars:
    NumOfVars = 3
    b2ai, b2ah, b2ar = range(NumOfVars)

    Name = {0: 'b2ai', 1: 'b2ah', 2: 'b2ar'}
    command = Name

    Values = {'b2ai': 0, 'b2ah': 1, 'b2ar': 2}

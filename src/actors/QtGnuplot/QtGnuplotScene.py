from PySide6.QtWidgets import (QApplication, QGraphicsScene,
                             ...
                             QGraphicsRectItem)
from PySide6.QtGui import (QKeyEvent, QPolygonF, QPen, QBrush, QColor, QPixmap,
                         QPainterPath, QFont, QImage, QFontMetrics)
from PySide6.QtCore import (QDataStream, QPoint, QPointF, QRectF, Qt, QRect,
                          QLineF, QSizeF, QSize, QElapsedTimer)
...
class QtGnuplotScene(QGraphicsScene, QtGnuplotEventReceiver):
    ...
    def __init__(self, parent):
        ...
        self.m_currentZ = 1.0
        self.m_watches = [QElapsedTimer() for i in range(4)]
    ...
    def processEvent(self, ev_type: int, dataStream: QDataStream) -> None:
        logging.debug(f"GnuplotGraphicsScene processEvent {ev_type}")
        if ev_type != GEMove and ev_type != GEVector and not self.m_currentPolygon.isEmpty():
            logging.debug("Flushing current polygon")
            point = self.m_currentPolygon.last()
            self.flushCurrentPolygon()
            self.m_currentPolygon.append(point)
        if ev_type == GEClear:
            logging.debug("GnuplotGraphicScene::GEClear")
            self.resetItems()
        elif ev_type == GELineWidth:
            ...
            while i > 0:
                ...
                self.m_plot_group[i].setVisible(isVisible)
                self.m_key_boxes[i].setHidden(not isVisible)
        elif ev_type == GELayer:
            logging.debug("GnuplotGraphicScene::GELayer")
            self.flushCurrentPointsItem()
            layer = dataStream.readInt32()
            ...
            self.swallowEvent(ev_type, dataStream)
